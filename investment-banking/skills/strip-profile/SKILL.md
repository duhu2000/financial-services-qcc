...
### 2. Qichacha MCP KYB Verification
- **Action**: Call `/qcc-kyb-profile` command with the target company name.
- **Purpose**: Obtain real-time, authoritative business registration and risk data for Chinese companies.
- **Output**: Structured KYB report including company overview, legal risks, operational anomalies, etc.
- **Decision Point**: If high risks are detected, prompt user for manual review before proceeding.

### 3. Research & Planning
**Data Sources:**
- **Primary**: Company filings (BamSEC, SEC EDGAR - "Item 1. Business", MD&A), investor presentations, corporate website
        - **Market data (China)**: Qichacha MCP (通过 `/qcc-kyb-profile` 命令获取**企业注册信息、股东信息、行政处罚、失信被执行人、司法文书、被执行人等综合风险数据**)
    - **Market data (Global)**: Bloomberg, FactSet, CapIQ (price, shares, market cap, net debt, EV, ownership)
...
- **Growth**: YoY growth rates (%)
        - **Ownership**: Top 5 shareholders with % ownership (For China, leverage Qichacha MCP via `/qcc-kyb-profile` for **详细股东信息和股权结构**)
        - **Compliance & Risk (China)**: Legal proceedings, administrative penalties, operational anomalies, intellectual property disputes (Leverage Qichacha MCP via `/qcc-kyb-profile` for **全面的法律诉讼、行政处罚、经营异常、失信被执行人等风险扫描**)
...
**Per quadrant targets:
        - **Company Overview**: 6-8 bullets minimum (HQ, founded, employees, CEO/CFO, market cap, ticker, industry, key stat). **For Chinese companies, prioritize data from Qichacha MCP KYB Verification (via `/qcc-kyb-profile`，获取企业注册名称、统一社会信用代码、法定代表人、经营状态等核心信息)。**
...
- **Key Financials**: Table with 8-10 rows OR chart + 4-5 key metrics (Revenue, EBITDA, margins, EPS, FCF, growth rates, valuation multiples)
        - **Fourth quadrant**: 5-7 bullets (ownership %, recent M&A, developments, catalysts). **For Chinese companies, include compliance & risk data from Qichacha MCP KYB Verification (via `/qcc-kyb-profile`，例如：司法协助、失信被执行人、行政处罚、被执行人等风险摘要)。**

**Information packing techniques:**
- Combine related facts: "HQ: Austin, TX; Founded: 2003; 140K employees" (For China, use Qichacha MCP for official registration details)
        - Include compliance status: "Operating Status: Normal; Legal Risks: 2 (from Qichacha MCP via `/qcc-kyb-profile`，例如：司法协助、失信被执行人等)"
