...
1. 每个新会话首次执行本 SKILL 时，如客户端支持 MCP Resources，先执行资源发现并读取 `qcc://skills/index`、`qcc://terminology/core`、`qcc://policy/data-discipline`、`qcc://policy/entity-anchoring` 与 `qcc://skill/litigation-analysis/tool-binding`。
...
**裁判文书详情下钻（列表先行 → 按需单篇）**：重点案件的"判决结果 / 涉案金额 / 说理"必须以原文为准——先由维度一/二的列表工具（`get_judicial_documents`）拿到该案「文书ID」→ 仅对重点案件单篇调 `mcp__qcc-risk__get_judicial_document_detail`（section 默认 `core` = 本院认为 + 本院查明 + 判决结果；需诉辩主张传 `claims`、审理经过传 `process`）。**单轮 ≤ 3 篇**；判项 / 涉案金额一律引文书原文，不臆测、不做胜诉率预测（铁律 6）。
...
> **诉讼数量不单独定级。** 被诉是正常商业活动的一部分，对有一定规模的企业和平台型业务，被诉数量与偿债能力、合规意愿无关。定级只看**生效败诉应付金额**与**未结诉讼标的额**相对净资产的占比；案件数量、案由分布、结案方式作为固定列示的观察信息进入报告，不参与定级。
>
...
>
> **确定性计算约束**：败诉应付聚合金额、未结诉讼标的聚合金额及其净资产占比，只能引用服务端聚合结果或客户侧确定性计算器输出；模型不得跨案件自行累计或除法。没有确定性结果时，列示原始案件并把金额类评级标注为「待评分」。

- **A 级（可正常合作）**：无当前被执行人 / 失信；近 3 年生效败诉应付金额 < 净资产 1%；无未结重大诉讼（单案标的额均 < 净资产 5%）；核心人员无个人涉诉硬性失信。
- **B 级（可合作，加强监测）**：历史被执行但已全部履行；生效败诉应付金额占净资产 1%~5%；或有未结诉讼但单案标的额 < 净资产 10%；核心人员无当前失信。
- **C 级（谨慎合作，要求担保）**：当前有被执行人或 5 年内历史失信；或生效败诉应付金额占净资产 5%~20%；或有未结重大诉讼（单案标的额 ≥ 净资产 10%）；核心人员历史上有已解除的执行事件。
- **D 级（强烈建议避免）**：当前失信被执行人；或生效败诉应付金额 ≥ 净资产 20%；或存在群体性诉讼；或核心人员当前失信 / 限高 / 限出境。
- **F 级（绝对禁合作）**：确定性结果显示诉讼敞口已超偿付能力（生效败诉应付金额或未结诉讼标的聚合金额 ≥ 净资产 100%）；或实控人出走 / 刑事被告；或企业已注销 / 进入破产清算；任何合作关系都会带来诉讼牵连风险。
...
**综合诉讼风险评级：** {A / B / C / D / F} 级 · {可正常合作 / 加强监测 / 谨慎合作 / 强烈建议避免 / 绝对禁合作} · {一句话结论}
...
| 未结单案标的额 / 净资产 ≥ 10% | {是 / 否 / 待评分} | {只引用确定性结果；不估算判赔} |
| 群体性诉讼 | {是 / 否} | {列明案由、角色与案件状态；数量本身不定级} |
| 核心人员当前失信 / 刑事被告 / 出走 | {是 / 否} | {} |
| **综合评级** | **{A / B / C / D / F}** | {同时触发多项取最低，不取平均} |

### 8.2 决策建议

{可正常合作 / 加强监测 / 谨慎合作（要求担保）/ 强烈建议避免 / 绝对禁合作 + 条件；客观陈述，不替客户做最终裁定}
...
> **章节 ↔ 工具绑定**：执行摘要←全维度汇总；§1←各 Server 采集时间 + 工商基座；§2←`get_company_risk_scan` 先扫 + `get_judicial_documents` / `get_case_filing_info` / `get_hearing_notice` / `get_court_notice` / `get_service_notice` / `get_pre_litigation_mediation`（现状）+ qcc-history 同名历史工具（历史）；§3·§4←`get_judicial_documents`（按 `role` 取角色分布，须下钻）；§5←重点案件单篇 `get_judicial_document_detail`（section 默认 core · 单轮 ≤ 3 篇 · 不做胜诉预测）；§6←`get_judgment_debtor_info` / `get_dishonest_info` / `get_high_consumption_restriction` / `get_terminated_cases` / `get_equity_freeze` + qcc-history 历史执行工具；§7←`get_executive_risk_scan` 双锚先扫 + 命中维度 `get_executive_*` 原子下钻；§8←全维度汇总评级。
