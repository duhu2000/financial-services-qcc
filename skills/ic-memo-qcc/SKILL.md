...
**第六，股比聚合值全篇逐字引用且不得混淆字段语义。** 直接持股、总持股（含间接）、最终受益股份、表决权必须复用接口返回的同一原始字符串并保留全部小数位；例如 `35.4938%` 是直接持股、`47.6955%` 是总持股（含间接）、`53.0011%` 是表决权，禁止把总持股误写成间接持股，禁止写成 `47.70%` / `53.00%`。摘要、正文、表格与结论均适用，禁止自行加总、相减、相乘、估算或四舍五入。
