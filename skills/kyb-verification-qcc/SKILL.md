...
description: >
  ...
  金融机构对公客户开户、授信审批、尽调、年检场景的主体自动化核验工具。输入企业名称或统一社会信用代码后，SKILL 自动完成主体真实性核验、工商信息核验、历史治理稳定性回溯、受益所有人穿透和企业风险扫描，输出 KYB 合规底稿与可复核的参考评级。
...
- **第 3 步 · 下钻**：仅对"有风险关联方"按需单点下钻（关联方是企业 → qcc-risk 原子 / `get_company_risk_scan`；自然人 → executive 双锚）。**禁对返回的关联方再自动逐个 scan / 原子**（单层预警终点）。
...
**合规瑕疵显性披露**：只要存在任一当前处罚记录，报告 §9 必须列明工具实际返回的字段；未返回的处罚事由、种类、履行或公示状态写"未披露"，不得补造。事件评级上限用于防止正向分数把当前监管信号完全抵消。
...
| 直接控股股东、实际控制人或 UBO 控制企业命中任一重点维度                              | 每个去重事件 −10，合计上限 −20 |
| 仅能定位到一般一跳关联，无法确认控制关系                                             | 0；只列示关联预警，不自动扣分  |

> 同一关联事件不再额外叠加"当前失信"扣分。关联方普通民商事纠纷不扣分；关联扫描是单层预警终点，不自动穿透。
...
> **填写纪律（务必遵守，对齐本 SKILL 已有铁律）**：① §3「受益所有人 / 实际控制人」表逐字引用接口聚合值（最终受益股份 / 表决权 / 总持股），**禁自算穿透路径百分比、禁臆测中间层**（见「穿透零重构铁律」）；② §5 风险**先扫后钻**，定性必须有下钻明细，否则只写计数 +「（未取明细）」；③ §6 关联**单层预警**、不替客户决策；④ 数值一律「数据零重构」（禁相乘 / 加总 / 四舍五入圆场）。
...
> 本栏不影响评级分数，但必须显性披露，并在 §9.4 列为条件项。无在册处罚时写"未发现在册行政处罚"。
...
> **章节 ↔ 工具绑定**：执行摘要←全维度汇总；§3←`verify_company_accuracy` / `get_company_registration_info` / `get_shareholder_info`；§4←治理历史与历史风险对应工具；§5←`get_beneficial_owners` / `get_actual_controller` / `get_key_personnel` + 个人风险先扫后钻；§6←`get_company_risk_scan` 先扫 + 命中维度原子下钻；§7←`get_company_related_risk_scan` / `get_executive_related_risk_scan`；§8←qcc-operation。
