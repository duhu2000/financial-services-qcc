...
description: >
  ...
  支持"单人深度核查 / 核心管理层批量核查 / 全员董监高扫描"三种模式，针对目标企业的法定代表人、董事长、总经理、核心董监高、实际控制人开展个人风险穿透与关联网络测绘。

  三种使用模式：
  - **默认模式**（不传参数）：自动核查"法定代表人 + 实际控制人 + 董事长 + 总经理"四位核心管理层，其他董监高仅列名单
  - **单人深度模式**（`--person 姓名`）：仅对指定一人做 18 现状 + 14 历史工具完整画像，适合人才引进 / 高管聘任场景
  - **全员模式**（`--depth full`）：对全部董监高（含监事、独董、职工董事）做完整画像，适合 IPO 申报 / 投前 DD / 客户准入等需要完整人员合规画像的场景
  ...
  适用场景：投资尽调（建议 --depth full）/ 人才引进（建议 --person 单人）/ 高管聘任（建议 --person 单人）/ IPO 核查（建议 --depth full）/ 反洗钱 KYC（默认即可）/ 客户风险画像（默认即可）。
