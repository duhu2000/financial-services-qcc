...
**第六，股比类数字全篇逐字引用且零缩位。** 凡直接持股、总持股、最终受益股份、表决权在执行摘要、一句话结论、正文或表格中出现，每一次都必须复用接口返回的同一原始字符串并保留全部小数位；例如接口返回 `47.6955%` / `53.0011%` 时，禁止写成 `47.70%` / `53.00%`。禁止自行加总、相减、相乘、加权、估算或四舍五入。
